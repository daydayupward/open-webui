...
async def summarize_node(state: MetricsSubgraphState) -> dict:
    ...
    for i in range(max_attempts + 1):
        messages = [system_prompt, user_prompt] + [m for m in local_messages if m.type != "system"]
        ...
        if i < max_attempts:
            feedback_prompt = (
                "\n\n[System Alert]: Your summary of the metrics/documents was either incomplete "
                "or not fully supported by the retrieved data. Please rewrite the summary, "
                "basing it strictly on the query results and reference texts without introducing hallucinated claims."
            )
