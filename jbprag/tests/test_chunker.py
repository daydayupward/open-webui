"""Tests for jbprag.src.ingestion.chunker."""
...
class TestSplitLongParagraph:
    def test_short_text_unchanged(self):
        ...
        assert _split_long_paragraph("Short text.", max_tokens=100, encoder=encoder) == ["Short text."]

    def test_splits_on_sentences(self):
        encoder = tiktoken.get_encoding("cl100k_base")
        text = "First sentence. Second sentence. Third sentence."
        parts = _split_long_paragraph(text, max_tokens=4, encoder=encoder)
        assert len(parts) > 1
    ...
    def test_hard_split_on_no_boundary(self):
        encoder = tiktoken.get_encoding("cl100k_base")
        text = "word " * 250
        parts = _split_long_paragraph(text, max_tokens=100, encoder=encoder)
        assert len(parts) == 3
...
class TestChunkDocument:
    ...
    def test_tables_are_not_split(self, table_doc):
        chunks = chunk_document(table_doc, max_chunk_tokens=50)
    ...
    def test_long_paragraph_is_split(self, long_paragraph_doc):
        chunks = chunk_document(long_paragraph_doc, max_chunk_tokens=200)
        assert len(chunks) > 1
    ...
    def test_overlap_prepends_context(self):
        ...
        chunks_no_overlap = chunk_document(doc, max_chunk_tokens=20, overlap_tokens=0)
        chunks_overlap = chunk_document(doc, max_chunk_tokens=20, overlap_tokens=10)
